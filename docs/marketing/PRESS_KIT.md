# MicroBreaks Press Kit

---

## Quick Facts

| | |
|---|---|
| **App Name** | MicroBreaks |
| **Tagline** | Smart Screen Break Reminders |
| **Category** | Health & Fitness / Productivity |
| **Platforms** | iOS 15+, Android 10+ |
| **Price** | Free |
| **Developer** | Can Kanbur |
| **Website** | https://microbreaks.app |
| **Contact** | press@microbreaks.app |

---

## About MicroBreaks

### Short Description (50 words)
MicroBreaks is a smart break reminder app that helps screen-heavy users protect their eyes, improve posture, and reduce stress through timely reminders and guided exercises. With guided routines for eyes, neck, back, wrists, and breathing, it makes healthy screen habits simple and achievable.

### Long Description (150 words)
In a world where we spend 7+ hours daily staring at screens, MicroBreaks is the wellness companion designed to protect what matters most - your health.

MicroBreaks uses intelligent scheduling to remind you when it's time to step away from the screen. Each break features quick, effective exercises designed by wellness experts: eye exercises to reduce digital strain, neck and shoulder stretches to release tension, back movements for better posture, wrist stretches to prevent repetitive strain, and breathing exercises for stress relief.

The app tracks your progress with streaks and achievements, making healthy habits rewarding and fun. Core routines work offline, no email or password sign-up is required, and privacy-first sync keeps your data available across devices only when you want it.

Whether you're a remote worker, student, gamer, or anyone who spends extended time on screens, MicroBreaks helps you build sustainable, healthy screen habits.

---

## Key Features

### Smart Reminders
- Customizable break intervals (20 min - 2 hours)
- Intelligent scheduling that adapts to your routine
- Do Not Disturb integration

### Guided Exercises
- 50+ exercises across 5 categories
- Eye exercises for digital strain relief
- Neck & shoulder stretches
- Back exercises for posture
- Wrist stretches for RSI prevention
- Breathing exercises for stress relief

### Progress Tracking
- Daily and weekly streaks
- Achievement system
- Break history and patterns
- Personalized insights

### Privacy-First Design
- Local-first data storage
- No email or password sign-up
- Optional cloud sync across devices
- Anonymous analytics and crash reporting for app stability

---

## The Problem We Solve

**Digital Eye Strain** affects 65% of screen users, causing:
- Headaches
- Blurred vision
- Dry eyes
- Neck and shoulder pain

**The 20-20-20 Rule** recommends:
> Every 20 minutes, look at something 20 feet away for 20 seconds.

MicroBreaks makes following this and other wellness recommendations effortless.

---

## Target Audience

- **Remote Workers** - 40 million+ in the US alone
- **Students** - Long study sessions require regular breaks
- **Gamers** - Extended gaming sessions cause eye strain
- **Office Professionals** - 8+ hours of daily screen time
- **Content Creators** - Hours of editing and creating

---

## Company Information

### About the Developer

MicroBreaks is developed by Can Kanbur, an independent developer based in Istanbul, Turkey, passionate about creating tools that improve daily life.

### Mission Statement

"To help everyone build healthier relationships with their screens through simple, effective break reminders."

---

## Brand Assets

### Logo
- Primary logo (PNG, SVG)
- App icon (all sizes)
- Monochrome variants

### Colors
| Color | Hex | Usage |
|-------|-----|-------|
| Primary Blue | #4A90D9 | Primary actions, branding |
| Secondary Teal | #34C4B5 | Accents, success states |
| Background Light | #E6F4FE | Light mode background |
| Background Dark | #1A1A2E | Dark mode background |

### Typography
- **Headings:** SF Pro Display (iOS) / Roboto (Android)
- **Body:** SF Pro Text (iOS) / Roboto (Android)

### Screenshots
- Available in: iPhone 6.7", 6.5", iPad Pro
- Languages: English, Turkish

---

## App Store Links

- **iOS:** [App Store Link - Coming Soon]
- **Android:** [Google Play Link - Coming Soon]

---

## Press Coverage

*Coming soon after launch*

---

## Testimonials

*Coming soon from beta testers*

---

## Media Contact

**Press Inquiries:**
- Email: press@microbreaks.app
- Website: https://microbreaks.app/press

**Social Media:**
- Twitter: @MicroBreaksApp
- Instagram: @microbreaks.app

---

## Download Assets

All press kit assets are available at:
**https://microbreaks.app/press-kit**

Includes:
- [ ] App icon (PNG, 1024x1024)
- [ ] Logo variations (PNG, SVG)
- [ ] Screenshots (all device sizes)
- [ ] App preview video
- [ ] Brand guidelines PDF
- [ ] Founder photo (if needed)

---

## FAQ for Press

**Q: Is MicroBreaks free?**
A: MicroBreaks' core features are currently available at no cost. Optional premium plans may be introduced in future updates.

**Q: Does MicroBreaks require an internet connection?**
A: Core routines work offline. Cloud sync, push notifications, and diagnostics require an internet connection.

**Q: What makes MicroBreaks different from other reminder apps?**
A: MicroBreaks combines smart reminders with guided exercises specifically designed for screen-related health issues. It's not just a timer - it's a complete wellness solution for screen users.

**Q: Is my data shared with third parties?**
A: We use infrastructure providers like Firebase for anonymous authentication, optional sync, analytics, and crash reporting. We do not sell personal data or use ad tracking.

---

*Last Updated: January 2025*
