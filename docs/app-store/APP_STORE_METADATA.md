# App Store Metadata / Uygulama Mağazası Verileri

---

## iOS App Store

### English (US/UK)

#### App Name (30 characters max)
```
Unwind: Desk Stretches
```

#### Subtitle (30 characters max)
```
Stretch, relax & refocus
```

#### Description (4000 characters max)
```
Take care of your eyes, body, and mind with Unwind - the intelligent break reminder app designed for people who spend hours in front of screens.

SMART BREAK REMINDERS
Unwind uses intelligent scheduling to remind you when it's time to take a break. Customize your break intervals from 20 minutes to 2 hours based on your work style.

GUIDED EXERCISES
Each break features quick, effective exercises designed by wellness experts:
• Eye exercises to reduce strain and prevent fatigue
• Neck and shoulder stretches to release tension
• Back exercises for better posture
• Wrist stretches to prevent repetitive strain
• Breathing exercises for stress relief

TRACK YOUR PROGRESS
• Build healthy habits with streaks and achievements
• View your break history and patterns
• Celebrate milestones with rewarding animations
• Get personalized insights about your break habits

WORKS THE WAY YOU WORK
• Flexible scheduling that adapts to your routine
• Do Not Disturb integration for focused work sessions
• Core routines work offline
• Beautiful, calming design that doesn't distract

PERFECT FOR
• Remote workers and home office professionals
• Students studying for long hours
• Gamers looking to prevent eye strain
• Anyone who spends extended time on screens

PRIVACY FIRST
Your core data stays on your device first. No email/password setup is required, optional cloud sync is available, and anonymous diagnostics help improve stability.

Download Unwind today and start building healthier screen habits. Your eyes, neck, and back will thank you!
```

#### Keywords (100 characters max)
```
break,reminder,eye,strain,posture,stretch,wellness,productivity,screen,health,ergonomic,rest,timer
```

#### Promotional Text (170 characters max)
```
Take smart breaks, feel better! Unwind reminds you to rest your eyes, stretch your body, and refresh your mind with quick, guided exercises.
```

#### What's New (4000 characters max)
```
Welcome to Unwind 1.0!

We're excited to launch the first version of Unwind. This release includes:

• Smart break reminders with customizable intervals
• 50+ guided exercises for eyes, neck, back, wrists, and breathing
• Progress tracking with streaks and achievements
• Beautiful animations and calming design
• Multi-language support (English & Turkish)
• Full offline functionality

Start your journey to healthier screen habits today!
```

---

### Turkish (TR)

#### App Name (30 characters max)
```
Unwind: Desk Stretches
```

#### Subtitle (30 characters max)
```
Esne, rahatla ve odaklan
```

#### Description (4000 characters max)
```
Unwind ile gözlerinize, vücudunuza ve zihninize iyi bakın - ekran başında uzun saatler geçiren herkes için tasarlanmış akıllı mola hatırlatıcı uygulama.

AKILLI MOLA HATIRLATICLARI
Unwind, mola zamanı geldiğinde sizi uyarmak için akıllı zamanlama kullanır. Çalışma tarzınıza göre mola aralıklarınızı 20 dakikadan 2 saate kadar özelleştirin.

REHBERLİ EGZERSİZLER
Her mola, sağlık uzmanları tarafından tasarlanmış hızlı ve etkili egzersizler içerir:
• Göz yorgunluğunu azaltmak için göz egzersizleri
• Gerginliği gidermek için boyun ve omuz germe hareketleri
• Daha iyi duruş için sırt egzersizleri
• Tekrarlayan zorlanmayı önlemek için bilek germesi
• Stres atmak için nefes egzersizleri

İLERLEMENİZİ TAKİP EDİN
• Seriler ve başarılarla sağlıklı alışkanlıklar oluşturun
• Mola geçmişinizi ve kalıplarınızı görüntüleyin
• Ödüllendirici animasyonlarla kilometre taşlarını kutlayın
• Mola alışkanlıklarınız hakkında kişiselleştirilmiş içgörüler alın

SİZİN GİBİ ÇALIŞIR
• Rutininize uyum sağlayan esnek zamanlama
• Odaklanmış çalışma oturumları için Rahatsız Etmeyin entegrasyonu
• Çevrimdışı işlevsellik - internet gerekmez
• Dikkatinizi dağıtmayan güzel, sakinleştirici tasarım

KİM İÇİN İDEAL
• Uzaktan çalışanlar ve ev ofisi profesyonelleri
• Uzun saatler ders çalışan öğrenciler
• Göz yorgunluğunu önlemek isteyen oyuncular
• Ekran başında uzun zaman geçiren herkes

ÖNCELİK GİZLİLİK
Verileriniz cihazınızda kalır. Hesap gerekli değil. Bulut depolama yok. Takip yok.

Unwind'i bugün indirin ve daha sağlıklı ekran alışkanlıkları oluşturmaya başlayın. Gözleriniz, boyununuz ve sırtınız size teşekkür edecek!
```

#### Keywords (100 characters max)
```
mola,hatırlatıcı,göz,yorgunluk,duruş,germe,sağlık,verimlilik,ekran,ergonomi,dinlenme,zamanlayıcı
```

#### Promotional Text (170 characters max)
```
Akıllı molalar alın, kendinizi daha iyi hissedin! Unwind gözlerinizi dinlendirmenizi, vücudunuzu germenizi hatırlatır.
```

---

## Google Play Store

### English

#### Short Description (80 characters max)
```
Smart break reminders with guided exercises for eyes, posture & wellness
```

#### Full Description (4000 characters max)
```
Take care of your eyes, body, and mind with Unwind - the intelligent break reminder app designed for people who spend hours in front of screens.

★ SMART BREAK REMINDERS
Unwind uses intelligent scheduling to remind you when it's time to take a break. Customize your break intervals from 20 minutes to 2 hours based on your work style.

★ GUIDED EXERCISES
Each break features quick, effective exercises designed by wellness experts:
→ Eye exercises to reduce strain and prevent fatigue
→ Neck and shoulder stretches to release tension
→ Back exercises for better posture
→ Wrist stretches to prevent repetitive strain
→ Breathing exercises for stress relief

★ TRACK YOUR PROGRESS
→ Build healthy habits with streaks and achievements
→ View your break history and patterns
→ Celebrate milestones with rewarding animations
→ Get personalized insights about your break habits

★ WORKS THE WAY YOU WORK
→ Flexible scheduling that adapts to your routine
→ Do Not Disturb integration for focused work sessions
→ Core routines work offline
→ Beautiful, calming design that doesn't distract

★ PERFECT FOR
→ Remote workers and home office professionals
→ Students studying for long hours
→ Gamers looking to prevent eye strain
→ Anyone who spends extended time on screens

★ PRIVACY FIRST
Your core data stays on your device first. No email/password setup is required, optional cloud sync is available, and anonymous diagnostics help improve stability.

Download Unwind today and start building healthier screen habits. Your eyes, neck, and back will thank you!

Questions or feedback? Contact us at support@microbreaks.app
```

### Turkish

#### Short Description (80 characters max)
```
Göz, duruş ve sağlık için rehberli egzersizlerle akıllı mola hatırlatıcı
```

---

## App Store Categories

### Primary Category
```
Health & Fitness
```

### Secondary Category
```
Productivity
```

---

## Age Rating

### iOS
```
4+ (No objectionable content)
```

### Android
```
Everyone
```

---

## App Store URLs (To be configured)

```
Privacy Policy: https://microbreaks.app/privacy
Terms of Service: https://microbreaks.app/terms
Support URL: https://microbreaks.app/support
Marketing URL: https://microbreaks.app
```

---

## Screenshot Text Overlays

### Screen 1: Hero
```
EN: "Smart Breaks for Healthier Screen Time"
TR: "Daha Sağlıklı Ekran Süresi için Akıllı Molalar"
```

### Screen 2: Reminders
```
EN: "Get Reminded at the Perfect Time"
TR: "Tam Zamanında Hatırlatma Alın"
```

### Screen 3: Exercises
```
EN: "50+ Guided Exercises"
TR: "50+ Rehberli Egzersiz"
```

### Screen 4: Progress
```
EN: "Track Your Healthy Habits"
TR: "Sağlıklı Alışkanlıklarınızı Takip Edin"
```

### Screen 5: Categories
```
EN: "Eyes • Neck • Back • Wrists • Breathing"
TR: "Göz • Boyun • Sırt • Bilek • Nefes"
```

### Screen 6: Privacy
```
EN: "Your Data Stays Private"
TR: "Verileriniz Gizli Kalır"
```
